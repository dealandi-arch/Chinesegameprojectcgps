...
import {
  approveSlideEditRequest,
  rejectSlideEditRequest,
} from "@/app/actions/slides";
import type { SlideEditRequest } from "@/lib/slides";

export function PendingSlideRequestRow({
  request,
  proposerUsername,
  slideTitle,
}: {
  ...
  function approve(force: boolean) {
    ...
    startTransition(async () => {
      const result = await approveSlideEditRequest(request.id, force);
      if (result.error) {
  ...
  function reject() {
    setError(null);
    startTransition(async () => {
      const result = await rejectSlideEditRequest(request.id);
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs text-stone-500">
            {slideTitle ? `Edit to "${slideTitle}"` : "New slide"} by{" "}
